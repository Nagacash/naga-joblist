...
API route in app/api/agent/find
        ↓
Calls lib/adzuna.ts (searchJobs + formatAdzunaSalary)
        ↓
Adzuna API returns job listings
        ↓
GPT-4o batch-scores jobs against user profile (in route)
        ↓
Route writes results to InsForge DB (jobs + agent_runs.country_searched)
        ↓
...
| country_searched   | text        | Adzuna market code (us, gb, …) |
