...
const jobRecord = {
  ...
  salary: formatAdzunaSalary(job.salary_min, job.salary_max, country),
...
- Always display "Jobs by Adzuna" credit on job listings — 116x23px minimum linked to adzuna.com (`AdzunaAttribution` component)
- Format salaries with `formatAdzunaSalary()` — currency symbol matches the Adzuna country market
- Adzuna description is a snippet — GPT-4o scores from it, not a full description
- Default country to `'us'` — Adzuna supports 12 markets: `us`, `gb`, `ca`, `au`, `de`, `fr`, `at`, `pl`, `in`, `nz`, `br`, `za`
