...
Defined in `app/globals.css`. Use these instead of ad-hoc `bg-surface shadow-card` on premium surfaces.

| Class           | Use case                                                                 |
| --------------- | ------------------------------------------------------------------------ |
| `glass-panel`   | Primary cards — dashboard stats, tables, profile sections, search form   |
| `glass-subtle`  | Nested stat tiles (job info mini-cards)                                  |
| `glass-nav`     | Sticky navbar — frosted header with blur                                 |
| `glass-field`   | Inputs/selects on glass panels                                           |
| `app-ambient`   | Page `<main>` background — soft gold/blue gradient mesh                    |

**Pattern notes:**
All glass styles use `color-mix()` with design tokens (no hardcoded hex in components). Includes `@supports` fallback to solid `bg-surface` when backdrop-filter is unavailable.
...
| Property         | Class                                                                                                   |
| ---------------- | ------------------------------------------------------------------------------------------------------- |
| Background       | `glass-panel` outer shell with `landing-hero-glow` on the left auth storytelling panel                   |
| Border           | `border-b border-border` on mobile split, `lg:border-r` on desktop split (glass border from utility)    |
...
| Shadow           | `var(--shadow-glass)` via `glass-panel`                                                                 |
...
File: components/homepage/LandingMediaFrame.tsx
Last updated: 2026-06-07

| Property         | Class                                                                                                          |
| ---------------- | -------------------------------------------------------------------------------------------------------------- |
| Shell            | `landing-browser-frame landing-browser-shadow mx-auto w-full max-w-[1114px] overflow-hidden rounded-[26px] bg-surface` |
| Chrome bar       | `landing-video-chrome` with three `landing-video-chrome-dot` controls                                          |
| Usage            | Hero video, How It Works screenshot, Features screenshot — one shared product-preview frame across the landing page |

**Pattern notes:**
All landing media (video + stills) share this glass browser shell so the page reads as one cohesive product story.

### Hero Video

File: components/homepage/HeroVideo.tsx
Last updated: 2026-06-07

| Property    | Detail                                                                 |
| ----------- | ---------------------------------------------------------------------- |
| Type        | Client leaf component — motion/accessibility only                      |
| Motion      | Honors `prefers-reduced-motion: reduce` (pauses on first frame)        |
| Video attrs | `muted`, `loop`, `playsInline`, `preload="metadata"`, `aria-label`     |
...
| Property         | Class                                                                                                          |
| ---------------- | -------------------------------------------------------------------------------------------------------------- |
| Background       | `landing-panel landing-hero-glow` unified shell (copy + clip)                                                  |
| Semantics        | `section[aria-labelledby=hero-heading]`, `figure` + `figcaption.sr-only` around media                          |
| Typography       | `landing-display` on h1, `landing-lead` on subtitle (`text-wrap: balance` / `pretty`, `max-width: 65ch`)       |
| Border radius    | `landing-button-*` on buttons; media via `LandingMediaFrame` (`rounded-[26px]`)                               |
...
| Spacing          | Optical bottom padding `pb-20 sm:pb-24 lg:pb-28`; clip gap `mt-12 sm:mt-14 lg:mt-16`                           |
| Hover/active     | `landing-button-primary` / `secondary` with `:active` scale feedback                                         |
| Shadow           | `landing-browser-shadow` via `LandingMediaFrame`; inset refraction on `landing-hero-video-stage::after`        |
...
| Hover state      | `none`                                                                                         |
| Shadow           | `landing-browser-shadow` via shared `LandingMediaFrame` on both sections                    |
...
Files: `app/privacy/page.tsx`, `app/terms/page.tsx`, `components/legal/LegalPageShell.tsx`, `components/legal/LegalSection.tsx`, `lib/legal.ts`
Last updated: 2026-06-07

| Property    | Class / detail                                                                 |
| ----------- | ------------------------------------------------------------------------------ |
| Shell       | `LegalPageShell` — `landing-panel mx-auto max-w-3xl`, skip link, navbar, footer |
| Sections    | `LegalSection` — `scroll-mt-24`, `h2 text-lg font-semibold`, body `text-sm leading-7 text-text-secondary` |
| Links       | `text-accent` external contact + internal cross-links between privacy/terms     |
| Contact     | Maurice Holda — `CONTACT_EMAIL` + `CONTACT_LINKEDIN` from `lib/legal.ts`     |

**Pattern notes:**
Legal content uses semantic `article` + numbered `section` blocks. Footer links route to `/privacy` and `/terms`. Not legal advice — aligned to actual stack: InsForge, OAuth, Adzuna, OpenAI, Browserbase, PostHog.

### Site Credits

File: components/layout/SiteCredits.tsx
Last updated: 2026-06-07

| Property    | Class / detail                                                                 |
| ----------- | ------------------------------------------------------------------------------ |
| Text        | `text-center text-xs leading-6 text-text-muted`                                |
| Links       | `font-medium text-accent transition-opacity hover:opacity-80`                  |
| Content     | Jobs by Adzuna · Powered by Naga Apparel (insta: naga_apparel) · Design by Naga Codex |
| Props       | `includeAdzuna` — full line in footer; `false` under Adzuna logo on Find Jobs  |
...
| Border           | `border-x border-b border-border`; credits row `border-t border-border`               |
| Border radius    | `none`                                                                                |
| Text — primary   | `text-sm font-medium text-text-secondary`                                             |
| Text — secondary | `text-sm font-medium text-text-secondary`; credits via `SiteCredits`                  |
| Spacing          | Main `px-6 py-10 sm:px-8 lg:px-10`; credits `px-6 py-4 sm:px-8 lg:px-10`              |
| Hover state      | `hover:text-text-primary` on nav links                                                |
| Shadow           | `none`                                                                                |
| Accent usage     | `text-accent` on credit links                                                         |

**Pattern notes:**
Footer stays minimal and horizontal at larger sizes. `SiteCredits` sits in a bordered strip below nav links on every page that renders `Footer`.
...
| Background       | `bg-surface`                                                                          |
| Border           | `border border-border`                                                                |
| Border radius    | `rounded-2xl` card, `rounded-lg` inputs, select, and button                          |
...
| Accent usage     | `bg-accent text-accent-foreground` button; `focus:ring-accent` inputs/select; `bg-success-lightest border-success-light text-success-foreground` success banner |
| Error banner     | `rounded-md border border-error bg-surface text-sm font-medium text-error` (matches `LoginCard`) |

**Pattern notes:**
Four-column grid at `lg:` breakpoint — job title (with search icon), location, country `<select>` grouped by region (Americas, Europe, Asia-Pacific, Africa; all 12 Adzuna markets), Find Jobs button aligned to bottom. Location input auto-suggests country via `detectCountryFromLocation` unless user manually changed the selector. Helper text under country select uses Naga Codexx branding and explains unsupported EU markets and Dublin/UK index caveat. Error banner shown above success banner when present. Success banner uses `✨` emoji + green pill. Button disabled when job title empty or search in progress.

---

### AdzunaAttribution

File: components/find-jobs/AdzunaAttribution.tsx
Last updated: 2026-06-07

| Property      | Class                                                                 |
| ------------- | --------------------------------------------------------------------- |
| Layout        | `flex justify-center py-2`                                            |
| Container     | `inline-flex min-h-[23px] min-w-[116px] items-center gap-1 text-sm`   |
| Jobs link     | `font-medium text-accent hover:opacity-80` → adzuna.com               |
| Logo          | `/images/adzuna-logo.svg`, `h-[23px] w-auto` (linked to adzuna.com)   |

**Pattern notes:**
Required Adzuna API attribution — "Jobs" hyperlinked + logo image hyperlinked, minimum 116×23px. Rendered below jobs table when `totalCount > 0`.
...
| Axis labels   | `fill: "var(--color-text-muted)"`, `fontSize: 12`, `axisLine={false}`, `tickLine={false}` |
...
| Empty state   | `mt-5 text-sm text-text-muted` helper copy when all chart counts are zero       |

**Pattern notes:**
Three named exports from one file — `CompanyResearchChart`, `JobsOverTimeChart`, `MatchDistributionChart`. All are `"use client"` (recharts needs browser). Colors use CSS variable references (`var(--color-*)`) so they stay token-driven inside recharts props. Left margin is `left: -20` on all charts to trim excess YAxis whitespace. Area gradient defined in `<defs>` with id `jobsGradient`. Chart data comes from PostHog HogQL queries in `lib/posthog-analytics.ts`; empty states render instead of charts when there is no event data yet.
...
| Property   | Value                                                                        |
| ---------- | --------------------------------------------------------------------------- |
| Emblem     | `next/image` `src="/images/logo.png"` — NAGA gold/black cobra, 36×36         |
| Emblem cls | `h-9 w-9 rounded-lg object-contain`                                          |
| Wordmark   | `JobPilot` — `text-[19px] font-bold leading-7 tracking-tight text-text-darkest` |
| Link       | wraps both, `inline-flex items-center gap-2.5`, `aria-label="JobPilot home"`, `href="/"` |
| Props      | `href?` (default `/`), `priority?` (default `false`)                         |
| Used by    | `Navbar` (with `priority`), `Footer`                                         |
| Favicon    | `app/icon.png` (256×256, same cobra emblem)                                  |

**Pattern notes:**
Replaced the original gradient-tile placeholder logo (`/logo.png`) with the real brand cobra from the images folder. Emblem source is square-cropped to 256 for the favicon. Wordmark uses the documented Logo-text token (19px / 700 / text-text-darkest).
