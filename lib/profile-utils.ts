import type { Education, MissingField } from "@/types";

/** Supports legacy single-object rows stored before multi-education. */
export function normalizeEducation(
  raw: Education | Education[] | null | undefined,
): Education[] {
  if (!raw) return [];
...
export function calculateCompletion(data: CompletionInput): CompletionResult {
  const checks: Array<{ field: MissingField; filled: boolean }> = [
    ...
    {
      field: "EDUCATION",
      filled: (data.education ?? []).some((e) => !!e.degree),
    },
  ];
